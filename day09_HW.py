...
def bfs(g, current, find_vtx, visited):
    visited.append(current)  # 현재 노드를 방문한 목록에 추가
    queue = deque([current])  #current값을 가진 queue 생성
    while queue:    #지금 큐가 빌 때까지 반복
        i = queue.popleft()
        if i == find_vtx: # 찾고자 하는 정점이면 True 반환
            return True
        # 현재 노드(i)와 연결된 모든 노드 탐색
        for vertex in range(g.SIZE):
            if g.graph[i][vertex] != 0 and vertex not in visited:
                queue.append(vertex)  # 방문할 노드를 큐에 추가
                visited.append(vertex)  # 방문 목록에 추가
    return False #모든 탐색 후에도 목표를 찾지 못했다면 False


# 특정 정점이 그래프 내에서 연결되어 있는지 확인하는 함수
def find_vertex(g, find_vtx):
    visited = []
    return bfs(g, 0, find_vtx, visited)  # 0번 정점(춘천)에서 시작하여 탐색
